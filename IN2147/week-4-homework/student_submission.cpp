...
void findPathWithExhaustiveSearch(ProblemData &problemData, int timestep,
    ...
    // TODO: This needs to be parallelized.
    #pragma omp parallel for reduction(+: numPossiblePositions)
    for (int x = 0; x < MAP_SIZE; ++x) {
        for (int y = 0; y < MAP_SIZE; ++y) {
