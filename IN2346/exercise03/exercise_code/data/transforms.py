...
def compute_image_mean_and_std(images):
    ...
    channel_size = 1 if len(images.shape) == 3 else images.shape[3]

    mean = np.ones(channel_size)
    std = np.ones(channel_size)
    for i in range(channel_size):
        mean[i] = np.mean(images[:,:,i]) if channel_size == 1 else np.mean(images[:,:,:,i])
        std[i] = np.std(images[:,:,i]) if channel_size == 1 else np.std(images[:,:,:,i])
...
class NormalizeTransform:
    ...
    def __call__(self, images):
        ...
        ########################################################################
        self.mean, self.std = compute_image_mean_and_std(images)
