...
class ShapeNet(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        sdf_sign = np.where(input_sdf < 0, -1, 1)
        input_sdf = np.stack((np.abs(input_sdf), sdf_sign))
    ...
    def move_batch_to_device(batch, device):
        ...
        batch['input_sdf'] = batch['input_sdf'].to(device=device, dtype=torch.float)
        batch['target_df'] = batch['target_df'].to(device=device, dtype=torch.float)

    @staticmethod
    def get_shape_sdf(shapenet_id):
        sdf = None
        # TODO implement sdf data loading
        shape_path_sdf = f"{ShapeNet.dataset_sdf_path}/{shapenet_id}.sdf"
        sdf_shape = np.fromfile(shape_path_sdf, count=3, dtype="uint64")
        sdf = np.fromfile(shape_path_sdf, count=-1, offset=24, dtype="float32")
        sdf = sdf.reshape((sdf_shape[0],sdf_shape[1],sdf_shape[2]))
        return sdf
    ...
    def get_shape_df(shapenet_id):
        ...
        shape_path_df = f"{ShapeNet.dataset_df_path}/{shapenet_id}.df"
        df_shape = np.fromfile(shape_path_df, count=3, dtype="uint64")
        df = np.fromfile(shape_path_df, count=-1, offset=24, dtype="float32")
        df = df.reshape((df_shape[0], df_shape[1], df_shape[2]))
        return df
