...
class ShapeImplicit(torch.utils.data.Dataset):
    ...
    def get_sdf_samples(self, path_to_sdf):
        ...
        indices_pos = torch.randperm(pos_tensor.shape[0])[:int(self.num_sample_points/2)]
        indices_neg = torch.randperm(neg_tensor.shape[0])[:int(self.num_sample_points/2)]
        return torch.vstack((pos_tensor[indices_pos], neg_tensor[indices_neg])).to(torch.float32)
