...
class InferenceHandlerDeepSDF:
    ...
    def reconstruct(self, points, sdf, num_optimization_iters):
        ...
        for iter_idx in range(num_optimization_iters):
            ...
            x_in = torch.hstack((latent_codes, batch_points))
            #print(f"Shape of x_in: {x_in.shape}")
            predicted_sdf = model(x_in)
            #print(f"Shape of predicted_sdf (before): {predicted_sdf.shape}")

            # TODO: truncate predicted sdf between -0.1, 0.1
            predicted_sdf = torch.clamp(predicted_sdf, min=-0.1, max=0.1)
            #print(f"Shape of predicted_sdf (after): {predicted_sdf.shape}")
            #print(f"Shape of batch sdf: {batch_sdf.shape}")
    ...
    def interpolate(self, shape_0_id, shape_1_id, num_interpolation_steps):
        ...
        for i in range(0, num_interpolation_steps + 1):
            ...
            a = latent_codes[0, :]
            b = latent_codes[1, :]
            step_size = 1/num_interpolation_steps
            interpolated_code = a + (b-a) * step_size * i
