...
def signed_distance_sphere(x, y, z, r, x_0, y_0, z_0):
    ...
    # ###############
    f = np.sqrt((x - x_0) ** 2 + (y - y_0) ** 2 + (z - z_0) ** 2) - r
    return float(f) if not isinstance(x, (list, tuple, np.ndarray)) else f
...
def signed_distance_atom(x, y, z):
    ...
    # raise NotImplementedError
    # ###############
    f_1 = signed_distance_sphere(x,y,z,proton_radius,proton_center[0],proton_center[1],proton_center[2])
    R = orbit_radius
    r = orbit_thickness
    f_2 = signed_distance_torus(x,y,z,R,r,proton_center[0],proton_center[1],proton_center[2])
    f_3 = signed_distance_sphere(x,y,z,electron_radius,electron_center[0],electron_center[1],electron_center[2])
    f = np.minimum.reduce([f_1,f_2,f_3])
    return float(f) if not isinstance(x, (list, tuple, np.ndarray)) else f